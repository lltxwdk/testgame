...
bool ParsePdu::parsePduHead(char* in_buf, uint16_t inbuf_len, uint16_t& cmd_type, uint8_t &id, char*& data, uint16_t& data_len)
{
    if (!isOwnProtocol(in_buf, inbuf_len)) {
        return false;
    }
...
int ParsePdu::packPduBuf(char* data, uint16_t data_len, uint16_t cmd_id, uint8_t id, char* out_buf)
{
    *out_buf = 'S';
    *(out_buf + 1) = 'R';
    *(out_buf + 2) = 0;
