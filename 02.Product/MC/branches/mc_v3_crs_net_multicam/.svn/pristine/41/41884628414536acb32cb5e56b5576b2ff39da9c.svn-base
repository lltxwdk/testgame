...
public:
	...
	SR_uint32   m_max_bandwidth;//netmp可承载的最大带宽
	SR_uint32   m_max_terms;//netmp可承载的最大终端数
	std::string   m_ip;
...
private:
	...
	SR_char m_stdsystime[256];//系统授权标准终端的时间
	SR_char m_recsystime[256];//系统授权录制的时间
	SR_char m_livesystime[256];//系统授权直播的时间
	SR_char m_monitorsystime[256];//系统授权监控的时间
	SR_char m_voicesystime[256];//系统授权语音的时间
	...
	SR_void processCmdStopSendAssistVideo(MeetingRoom* pconftmp, SRTer* pter, const SRMsgs::CmdStopSendAssistVideo *s, SR_uint32 uisponsortype);// 通知共享者停止共享
	...
	SR_uint32 chooseNonMapipsSameTypeNewNetMP(SR_uint32 errornetmpid, SR_uint32 errornetmpgroupid, SR_uint32 errornetmpnettype, std::string errornetmpip);//选择一空闲的相同nettype的netmp(没有设置映射地址列表),上面没有召开任何会议
...
typedef enum{
	e_sub_state_avpause = 0x0,//共享是否暂停(开启共享的前提下)
	e_sub_state_avlabel = 0x1,// 共享标注(开启共享的前提下)
	e_sub_state_avchairman = 0x2//只有主持人可以发起共享
}E_SubStatusType;
class TerInfo
{
public:
...
public:
	...
	SR_uint32 m_substatustype;//会议中功能的子功能类型,0-共享是否暂停(开启共享的前提下)、1-共享标注(开启共享的前提下)、2-只有主持人可以发起共享
	SR_uint32 m_substattus;//会议中功能的子功能对应状态，0-关闭、1-开启（substatustype==2时，0-代表所有人都可以发起共享，1-只有主持人可以发起共享）
	SR_uint32 m_assistvideotimes;//会议中共享时间，单位：秒；目前只支持substatustype==2时使用
};
