...
bool CConference::UpdateDB(int realtimetype)
{
	...
	{
		sr_printf(SR_LOGLEVEL_ERROR, "update conference error: %s\n", CDBConnector::GetInstance()->OutErrors());
		return false;
	}
...
bool CConference::SelectDB()
{
	...
	char cSelect[1024];
#ifdef WIN32
	sprintf(cSelect, "select conf_id,user_id,conf_passwd,recv_passwd,conf_subject,conf_type,conf_chairman,conf_createtm,conf_starttype,conf_starttime,conf_endtime,conf_maxterm,audio_enable,video_enable,permanent_enable,isdeleted,ctrl_user_id,ctrl_pwd,anonymous_enable,conf_name,encypt_alg,auto_rec,video_size from conference where conf_id='%I64d'", m_conf_id);
#elif defined LINUX
	sprintf(cSelect, "select conf_id,user_id,conf_passwd,recv_passwd,conf_subject,conf_type,conf_chairman,conf_createtm,conf_starttype,conf_starttime,conf_endtime,conf_maxterm,audio_enable,video_enable,permanent_enable,isdeleted,ctrl_user_id,ctrl_pwd,anonymous_enable,conf_name,encypt_alg,auto_rec,video_size from conference where conf_id='%lld'", m_conf_id);
#endif
