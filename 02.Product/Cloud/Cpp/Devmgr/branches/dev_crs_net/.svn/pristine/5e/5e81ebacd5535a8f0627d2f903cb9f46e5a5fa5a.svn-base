...
bool CSRedis::pingpong()
{
	...
	if (this->_connect != NULL)
	{
		...
		printf("xxxxxxxxxxx  CSRedis::pingpong Error:[%d, %s], this->_reply=%p\n", this->_connect->err, this->_connect->errstr, this->_reply);
...
bool CSRedis::selectdb(const int dbnum)
{
	...
	if (this->_connect != NULL)
	{
		...
		if (this->_reply != NULL)
		{
			...
			//printf("===== this->_reply != NULL========== CSRedis::selectdb(%d)=====------------freeReplyObject------------------>>>>>>>>>>\n", dbnum);
			return true;
		}
...
//bool CSRedis::command_array(RedisReplyArray& rArray, const char* cmd)
{
	...
	if (this->_connect != NULL)
	{
		...
		else
		{
			if (this->_reply != NULL)
			{
				printf("==2==command_array==this->_reply->type==-->>> %d\n", this->_reply->type);
...
bool CSRedis::commandargv_array(const REDISVDATA& vDataIn, RedisReplyArray& rArray)
{
	...
	if (this->_connect != NULL)
	{
		...
		else
		{
			if (this->_reply != NULL)
			{
				printf("=2=commandargv_array==this->_reply->type==-->>> %d\n", this->_reply->type);
...
bool CSRedis::commandargv_pair(const REDISVDATA& vDataIn, RedisReplyKeyValuePair& mPair)
{
	...
	if (this->_connect != NULL)
	{
		...
		else
		{
			if (this->_reply != NULL)
			{
				//printf("=2=commandargv_pair==this->_reply->type==-->>> %d\n", this->_reply->type);
...
bool CSRedis::hashHGetAll(const char* key, RedisReplyArray& rArray)
{
	...
	if (this->_connect != NULL)
	{
		...
		else
		{
			if (this->_reply != NULL)
			{
				printf("==2==hashHGetAll==this->_reply->type==-->>> %d\n", this->_reply->type);
