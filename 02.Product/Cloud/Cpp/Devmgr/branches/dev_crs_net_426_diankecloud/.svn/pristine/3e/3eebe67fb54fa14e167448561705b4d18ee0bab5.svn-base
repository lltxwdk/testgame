...
public:
	...
	SR_void One_wait_dispatch(){
		...
		for(int i = 0;i < retfds;i++){
			EventPoller* pdispatcher = (EventPoller*)(m_epevents[i].data.ptr);
...
public:
						 ...
						 TimerData* ptd)//注册的定时器数据
	{
		...
		if(m_timerfd <= 0){
			sr_printf(SR_LOGLEVEL_ERROR,"timerfd_create error %s\n",strerror(errno));
			return;
		}
		m_timertype = timertype;
		struct timespec now;   
		if (-1 == ::clock_gettime(CLOCK_MONOTONIC, &now)){
			sr_printf(SR_LOGLEVEL_ERROR,"clock_gettime error\n");
			::close(m_timerfd);
			return;
		}
		...
		if (-1 == ::timerfd_settime(m_timerfd, TFD_TIMER_ABSTIME, &new_value, NULL)){
			sr_printf(SR_LOGLEVEL_ERROR,"timerfd_settime error\n");
			::close(m_timerfd);
			return;
		}
		m_epoll_engine->AddPoller(this);
	}
