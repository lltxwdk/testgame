...
bool CDevMgr::Init()
{
	...
	m_pMainMpSysMtu = new CMpSysMtu();
	m_mapAsyncThreadMpSysMtu.insert(std::map<CAsyncThread*, CMpSysMtu*>::value_type(m_pMainThread, m_pMainMpSysMtu));
...
void CDevMgr::UpdateConfRealtime(unsigned long long confid, int realtimetype, char* time, time_t lltime, int permanentenable/*=0*/)
{
	...
	typedef CBufferT<unsigned long long, int, CAsyncThread*, time_t, int, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, realtimetype, m_pUpMsqThread, lltime, permanentenable);
...
void CDevMgr::InsertConfReport(unsigned long long confid, unsigned long long confreportid, char* time, time_t lltime)
{
	...
	typedef CBufferT<unsigned long long, unsigned long long, CAsyncThread*, time_t, void*, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, confreportid, m_pUpMsqThread, lltime);
...
void CDevMgr::UpdateDeviceConfDetail(unsigned long long confid, int deviceid, char* time, time_t lltime)
{
	...
	typedef CBufferT<unsigned long long, int, CAsyncThread*, time_t, void*, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, deviceid, m_pUpMsqThread, lltime);
...
void CDevMgr::InsertConfRecord(unsigned long long confid, unsigned long long confreportid, unsigned long long confrecordid, char* time, time_t lltime, char* confname)
{
	...
	typedef CBufferT<unsigned long long, unsigned long long, unsigned long long, CAsyncThread*, time_t, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), confname, strlen(confname), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, confreportid, confrecordid, m_pUpMsqThread, lltime);
...
void CDevMgr::InsertRecordFile(unsigned long long confid, unsigned long long confreportid, unsigned long long confrecordid, unsigned long long recordfileid, char* filestorsvrip, char* filerootpath, char* filepath, char* sdefilepath, time_t lltime, unsigned long long filesize)
{
	...
	typedef CBufferT<unsigned long long, unsigned long long, unsigned long long, unsigned long long, CAsyncThread*, time_t, unsigned long long, void*> CParam;
	CParam* pParam;
	pParam = new CParam(filestorsvrip, strlen(filestorsvrip), filerootpath, strlen(filerootpath), filepath, strlen(filepath), sdefilepath, strlen(sdefilepath), NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, confreportid, confrecordid, recordfileid, m_pUpMsqThread, lltime, filesize);
...
void CDevMgr::UpdateConfRecord(unsigned long long confid, unsigned long long confreportid, char* time, time_t lltime, char* confname)
{
	...
	typedef CBufferT<unsigned long long, CAsyncThread*, time_t, unsigned long long, void*, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), confname, strlen(confname), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, confid, m_pUpMsqThread, lltime, confreportid);
...
void CDevMgr::UpdateConfLiveInfo(const SRMsgs::IndCRSStopLive* ind, char* time, time_t lltime)
{
	...
	typedef CBufferT<unsigned long long, CAsyncThread*, time_t, void*, void*, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), strliveurl, strlen(strliveurl), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, ind->confid(), m_pUpMsqThread, lltime);
		...
		&& (uiConfliveinfoidsize % 2 == 0))
	{
		...
		if (liveinfoid != 0)
		{
				...
				&& vRedisReplyArray_liveinfo.size() == vGetFileds.size()) // 与push_back的个数一致
			{
				...
				//sprintf(conf_report_id_value, "%s", vRedisReplyArray_liveinfo[5].str.c_str()); // 需要与push_back插入顺序一致
...
void CDevMgr::ModifyConfCallListInfo(const SRMsgs::IndModifyConfCallList* ind, char* time, time_t lltime)
{
	...
	typedef CBufferT<unsigned long long, unsigned int, unsigned long long, CAsyncThread*, time_t, void*, void*, void*> CParam;
	CParam* pParam;
	pParam = new CParam(time, strlen(time), strrollcallname, strlen(strrollcallname), NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, NULL, 0, ind->confid(), uioptype, ullrollcallid, m_pUpMsqThread, lltime);
...
bool CDevMgr::ParseClientData(DeviceConnect *pClient, CAsyncThread* pThread, const char* pData, const int nLen, unsigned long recv_data_time)
{
	...
	*/
	...
	CMpSysMtu* pMpSysMtu = NULL;
	std::map<CAsyncThread*, CMpSysMtu*>::iterator iter_mapAsyncThreadMpSysMtuList = m_mapAsyncThreadMpSysMtu.find(pThread);
	if (iter_mapAsyncThreadMpSysMtuList != m_mapAsyncThreadMpSysMtu.end())
	{
		pMpSysMtu = iter_mapAsyncThreadMpSysMtuList->second;
	}
	...
	case SRMsgId_ReqRegister:
	{
		...
		if (req.ParsePartialFromArray(pData + 16, nLen - 16))
		{
			if (req.has_ip() && req.has_port() && req.has_auth_password() && req.has_svr_type() && req.has_cpunums())
			{
				...
				if (pClient != NULL)
				{
					bool bRegister = ProcessReqRegister(&req, pClient, pDeviceConfig, pDevice, pMpSysMtu, &rspsend);
...
bool CDevMgr::ProcessReqRegister(const SRMsgs::ReqRegister* req, DeviceConnect *pClient, CDeviceConfig* pDeviceConfig, CDevice* pDevice, CMpSysMtu* pMpSysMtu, SRMsgs::RspRegister* rspsend)
{
	...
	else
	{
		...
		if (pDevice->SelectDB1())
		{
			...
			else
			{
				...
				{
					pMpSysMtu->SelectMPSysMtu();

					//赋值给resp
					rspsend->set_sysmtu(pMpSysMtu->GetMPMaxMtu());

					sr_printf(SR_LOGLEVEL_INFO, "SRMsgId_ReqRegister pMpSysMtu->GetMPMaxMtu() %d success.\n", pMpSysMtu->GetMPMaxMtu());
				}
