...
void handler_http_cmdhjgw_opinvitepart_msg_cb(struct evhttp_request *req, void *arg)
{
	...
	CDevMgr* pdevMgr = NULL;
	CAsyncThread* pThread = NULL;
	ICsredis* pHttpRedisCoon = NULL;
	pdevMgr = (CDevMgr*)(pcbarg->m_arg1);
	pThread = (CAsyncThread*)(pcbarg->m_arg2);
	pHttpRedisCoon = (ICsredis*)(pcbarg->m_arg3);
	if (pdevMgr == NULL
		|| pThread == NULL
		|| pHttpRedisCoon == NULL)
	{
	...
	else
	{
		...
		if (pgroupid == NULL)
		{
			sr_printf(SR_LOGLEVEL_ERROR, "**handle opinvitepart http request*** error ****** get devmgrselfid:%s related groupid failed from db3 dev_deviceidandgroupid tab *****\n", devmgrselfidbuf);
			pHttpRedisCoon->selectdb(3);
			pgroupid = pHttpRedisCoon->getvalue("dev_deviceidandgroupid", devmgrselfidbuf);
			redisFlag = true;
		}
		...
		if (pgroupid != NULL)
		{
			...
			bool bHGetAllGwBeat_ok = false;
			unsigned int nDevidandBeatNum = 0;
			if (redisFlag)
			{
				pHttpRedisCoon->selectdb(1);

				bHGetAllGwBeat_ok = pHttpRedisCoon->hashHGetAll(svrname_groupid.c_str(), vrra_db1_deviceid_beat);
			...
			else
			{
				...
				bHGetAllGwBeat_ok = pdevMgr->m_pRedisConnList[e_RC_TT_HttpThread]->hashHGetAll(svrname_groupid.c_str(), vrra_db1_deviceid_beat);

				nDevidandBeatNum = vrra_db1_deviceid_beat.size();

			}
			sr_printf(SR_LOGLEVEL_INFO, "**handle opinvitepart http request ******redisFlag: %d get devmgrselfid:%s related db1 svrname_groupid(%s) failed *****bHGetAllGwBeat_ok=%d, nDevidandBeatNum=%d\n", devmgrselfidbuf, svrname_groupid.c_str(), bHGetAllGwBeat_ok, nDevidandBeatNum);
			...
			{
				sr_printf(SR_LOGLEVEL_ERROR, "**handle opinvitepart http request*** error ****** get devmgrselfid:%s related db1 svrname_groupid(%s) failed *****bHGetAllGwBeat_ok=%d, nDevidandBeatNum=%d\n", devmgrselfidbuf, svrname_groupid.c_str(), bHGetAllGwBeat_ok, nDevidandBeatNum);

			}
		}
		else
		{
			sr_printf(SR_LOGLEVEL_ERROR, "**handle opinvitepart http request*** error ***** *pHttpRedisCoon select, get devmgrselfid:%s related groupid failed from db3 dev_deviceidandgroupid tab *****\n", devmgrselfidbuf);
...
HttpConnMgr::HttpConnMgr(CAsyncThread* pThread, CDevMgr* devMgr, ICsredis* HttpRedisConn)
{
	m_pMainThread = pThread;
	m_devMgr = devMgr;
	http_conn_num_ = 0;
	m_pHttpRedisConn = HttpRedisConn;
}
...
//#elif defined LINUX
	...
	stcbarg_opinvitepart.m_arg3 = m_pHttpRedisConn;
