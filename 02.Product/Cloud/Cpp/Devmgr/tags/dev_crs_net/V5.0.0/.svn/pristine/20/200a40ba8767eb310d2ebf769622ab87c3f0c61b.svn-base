	...
	case SRMsgId_ReqRegister:
	{
		...
		if (req.ParsePartialFromArray(pData + 16, nLen - 16))
		{
			if (req.has_ip() && req.has_port() && req.has_auth_password() && req.has_svr_type() && req.has_cpunums())
			{
				...
				if (pClient != NULL)
				{
					bool bRegister = ProcessReqRegister(&req, pClient, pDeviceConfig, pDevice, pDataDictionary, pSysParameter, &rspsend);
...
bool CDevMgr::ProcessReqRegister(const SRMsgs::ReqRegister* req, DeviceConnect *pClient, CDeviceConfig* pDeviceConfig, CDevice* pDevice, CDataDictionary* pDataDictionary, CSysParameter* pSysParameter, SRMsgs::RspRegister* rspsend)
{
...
void CDevMgr::SplitString(string src, vector<unsigned int>& veclist, const std::string& cutFlag)
{
	std::string strtmp = src;
	size_t index;
	size_t lastindx = 0;
	index = strtmp.find(cutFlag);

	while (std::string::npos != index)
	{
		string strValue = strtmp.substr(lastindx, index - lastindx);
		unsigned int userid = 0;
		sscanf(strValue.c_str(), "%d", &userid);
		veclist.push_back(userid);

		lastindx = index + cutFlag.size();
		index = strtmp.find(cutFlag, lastindx);
	}
	if (lastindx != strtmp.length())
	{
		string strValue = strtmp.substr(lastindx);
		unsigned int userid = 0;
		sscanf(strValue.c_str(), "%d", &userid);
		veclist.push_back(userid);
	}
	...
	, CConfRollCallInfo* pConfRollCallInfo, CRollCallList* pRollCallList, CConfPollInfo* pConfPollInfo, CPollList* pPollList, DeviceConnect *pClient)
{
	...
	if (iMcid == req->deviceid())
	{
		...
		if (pConference->GetExist())
		{
			...
			//获取会议的charman_list
			std::string confCharmanList = pConference->GetChairmanList();
			vector<unsigned int> vCharmList;

			SplitString(confCharmanList, vCharmList, ";");
			sr_printf(SR_LOGLEVEL_DEBUG, "GetConfInfoFromDB confCharmanList=%s .\n", confCharmanList.c_str() );
			// 添加会议白名单
			SRMsgs::RspConfInfoToMC_WhiteListInfo* pAddWhiteListInfo = 0;
			pAddWhiteListInfo = rspsend.add_whitelists();
			if (pAddWhiteListInfo != 0)
			{
				pAddWhiteListInfo->set_whitelisttype(2);
				std::map<unsigned int, std::string>::iterator iter_participant;
				for (vector<unsigned int>::iterator it_chairid = vCharmList.begin(); it_chairid != vCharmList.end(); it_chairid++)
				{
					unsigned int chairmanUserid = *it_chairid;
					sr_printf(SR_LOGLEVEL_DEBUG, "GetConfInfoFromDB confCharmanid=%d ------chairmanUserid=%d.\n", *it_chairid, chairmanUserid);

					iter_participant = partuidnameinfos.find(chairmanUserid);
					if (iter_participant != partuidnameinfos.end())
					{
						SRMsgs::RspConfInfoToMC_ParticipantInfo* pAddcalllist = 0;
						pAddcalllist = pAddWhiteListInfo->add_parts();
						pAddcalllist->set_psuid(chairmanUserid);
						pAddcalllist->set_nickname(iter_participant->second.c_str());
					}
				}
			}
