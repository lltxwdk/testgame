...
bool CDevice::UpdateDB1()
{
	CCriticalAutoLock MyLock(m_csDevice);
	char cUpdate[1024];
	sprintf(cUpdate, "update device set max_terms = '%d', max_bandwidth = '%d', status = '%d', license = '%s', device_version = '%s' where device_id = '%d'", \
		m_max_terms, m_max_bandwidth, m_status, m_system_licence.c_str(), m_version.c_str(), m_device_id);
