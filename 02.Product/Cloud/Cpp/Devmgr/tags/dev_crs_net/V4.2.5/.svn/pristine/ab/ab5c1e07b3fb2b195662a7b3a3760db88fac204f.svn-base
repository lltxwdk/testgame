	...
	case SRMsgId_ReqRegister:
	{
		...
		if (req.ParsePartialFromArray(pData + 16, nLen - 16))
		{
			if (req.has_ip() && req.has_port() && req.has_auth_password() && req.has_svr_type() && req.has_cpunums())
			{
				...
				if (pClient != NULL)
				{
					bool bRegister = ProcessReqRegister(&req, pClient, pDeviceConfig, pDevice, &rspsend);
...
bool CDevMgr::ProcessReqRegister(const SRMsgs::ReqRegister* req, DeviceConnect *pClient, CDeviceConfig* pDeviceConfig, CDevice* pDevice, SRMsgs::RspRegister* rspsend)
{
